locals {
  ...
  deployer_appconfig_name                         = substr(replace(lower(format("%s%s%sapc%s", local.deployer_env_verified, local.deployer_location_short, local.dep_vnet_verified, local.random_id_verified)), "/[^a-z0-9]/", ""), 0, var.azlimits.stgaccnt)
