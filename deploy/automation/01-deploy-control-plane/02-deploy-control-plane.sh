...
start_group "Variables"
    ...
    if [ -n "${var}"  ]; then
        key_vault="${var}" ; echo 'Deployer Key Vault' ${key_vault}
    else
        if [ -f ${deployer_environment_file_name} ] ; then
            key_vault=$(config_value_with_key "keyvault") ; echo 'Deployer Key Vault' ${key_vault}
        fi
    fi

    var=$(get_value_with_key "Deployer_Terraform_Remote_Storage_Subscription" | tr -d \")
    if [ -n "${var}"  ]; then
        STATE_SUBSCRIPTION="${var}" ; echo 'Terraform state file subscription' $STATE_SUBSCRIPTION
    else
        if [ -f ${deployer_environment_file_name} ] ; then
            STATE_SUBSCRIPTION=$(config_value_with_key "STATE_SUBSCRIPTION") ; echo 'Terraform state file subscription' $STATE_SUBSCRIPTION
        fi
    fi

    var=$(get_value_with_key "Terraform_Remote_Storage_Account_Name.value" | tr -d \")
    if [ -n "${var}"  ]; then
        REMOTE_STATE_SA="${var}"  ; echo 'Terraform state file storage account' $REMOTE_STATE_SA
    else
        if [ -f ${deployer_environment_file_name} ] ; then
            REMOTE_STATE_SA=$(cat $(config_value_with_key "REMOTE_STATE_SA") ; echo 'Terraform state file storage account' $REMOTE_STATE_SA
        fi
    fi
